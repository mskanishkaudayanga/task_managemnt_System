  ...
  const onSubmit = async (data: LoginFields) => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        login(token, user);
        success("Welcome back!", `Hello, ${user.name}. You have successfully logged in.`);
        navigate("/");
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#f6f8fc] flex items-center justify-center p-4">
      ...
      >
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-[#0f172a]">Sign in to Taskly</h2>
          <p className="text-sm text-[#64748b] mt-1">Manage your team's workflow efficiently</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-elevation p-8">
          ...
          <div className="text-center mt-6 pt-4 border-t border-slate-100">
            <span className="text-xs text-[#64748b]">Don't have an account? </span>
            <Link
              to="/register"
